...
class Weapon(pygame.sprite.Sprite):
    def __init__(self, image: pygame.Surface, **kwargs) -> None:
        ...
        self.accuracy_max_angle = kwargs.get('accuracy_max_angle') or 35
        self.accuracy = kwargs.get('accuracy') or 1
        self.accuracy_recover_rate = kwargs.get('accuracy_recover_rate') or 0.93
        self.accuracy_current = self.accuracy
        self.recoil = kwargs.get('recoil') or 5
        self.cadency = kwargs.get('cadency') or 50
        self.last_shoot_time = 0

        self.bullet_speed = kwargs.get('bullet_speed') or 700
    ...
    def update(self, dt: float, new_pos: Vector2, pointer: Vector2) -> None:
        ...
        if self.accuracy_current > self.accuracy:
            if self.accuracy_current > self.accuracy_max_angle:
                self.accuracy_current = self.accuracy_max_angle
            else:
                self.accuracy_current *= self.accuracy_recover_rate
        else:
            self.accuracy_current = self.accuracy        
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        line_dir = vec(
            sin((self.angle - 90 + self.accuracy_current) * pi / 180),
            cos((self.angle - 90 + self.accuracy_current) * pi / 180),
        )
        
        pygame.draw.line(surface, (255, 255, 255), self.current_pos_canon, (line_dir * 100) + self.current_pos_canon)
    ...
    def shoot(self, bullets: 'list[Bullet]') -> None:
        ...
        for x in range(self.bullet_per_shoot):
            accuracy = randint(-int(self.accuracy_current), int(self.accuracy_current))
            bullet_dir = vec(
        ...
        self.accuracy_current *= self.recoil
        self.last_shoot_time = pygame.time.get_ticks()
        self.magazine_ammount -= self.ammo_per_shoot

        if self.magazine_ammount == 1:
